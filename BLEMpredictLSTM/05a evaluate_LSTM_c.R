...
for(id in files) {
    ...
    # Scale data
    scaled <- scaleData(unscaled, option = "log-normalize", time.tbl = FALSE)$data
    ...
    min_index_train = 1
    max_index_train = 1+ lookback + batch_size * steps * 700
    min_index_val   = max_index_train + 1    
    max_index_val   = max_index_train + 1 + lookback + batch_size * steps * 96
    ...
    callbacks <- list(
        ...
        callback_model_checkpoint(
            filepath = "models/checkpoint.hdf5",
    ...
    # for windows
    model_best %>% save_model_hdf5("models/consumption"%&%id%&%".hdf5")
    ...
    test_steps <- floor((max_index_test - min_index_test - lookback)/(5*batch_size))
    
    # Make predictions
    lstm_predictions <- numeric()
    pb               <- txtProgressBar(min = 0, max = test_steps, style = 3)
    idx              <- seq(1, (max_index_test-min_index_test)/5, by = batch_size)
    ...
    error_measures[[id]] <- calcErrorMeasure(lstm_predictions,
                                             targets,
                                             return = "all")
    
    # Save predictions to dataframe
    if(!exists("lstm_all_predictions")) {
        lstm_all_predictions <- matrix(NA, nrow = length(lstm_predictions), ncol = length(files))
        col <- 0
    }
    col <- col+1
